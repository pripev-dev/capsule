...
export function offlineHtml({ capsule, font, paths }) {
  ...
  if (!font || !/^[a-zA-Z0-9 -]+$/.test(font.family) ||
      ...
      !included.has(archivePath(font.path)) || !included.has(archivePath(font.licencePath))) {
  ...
  const fontCheck = `document.fonts.load('20px ' + getComputedStyle(document.body).fontFamily).then(fonts => { if (!fonts.length) throw new Error('font missing'); document.querySelector('main').hidden = false; document.querySelector('#font-status').hidden = true; }).catch(() => { document.querySelector('#font-status').textContent = 'The bundled typeface could not load. Keep the extracted archive together and check its integrity before reading or printing.'; });`;
...
@font-face{font-family:'${font.family}';src:url('../${font.path}') format('woff2');font-display:block}
...
<aside aria-label="Recordings">${audio}</aside>${transcripts}
